...
qlength_FS__ = np.zeros(10, dtype = float)
qlength_SS__ = np.zeros(10, dtype = float)
qlength_IS__ = np.zeros(10, dtype = float)
...
for k in range(10):
	...
	qlength_FS__[k] = qlength[1, num_vehicle]
	qlength_SS__[k] = qlength[num_station + 1, num_vehicle]
	qlength_IS__[k] = qlength[2 * num_station + 1, num_vehicle]
...
plt.figure()
line1 = plt.plot(num_server__, qlength_FS__, '*--r')
line2 = plt.plot(num_server__, qlength_SS__, '+-g')
line3 = plt.plot(num_server__, qlength_IS__, 'x:b')
plt.setp([line1, line2, line3], linewidth = 5.0)
plt.legend(('FS', 'SS', 'IS'), loc = 'upper right',fontsize = 40)
plt.xticks(fontsize=30)
plt.yticks(fontsize=30)
plt.xlabel('Number of Charger per Station', fontsize = 40)
plt.ylabel('Queue Length',fontsize = 40)
plt.show()