...
opti_m__= np.zeros(10, dtype = int)
'''
num_server__= np.zeros(10, dtype = float)
num_server__[0] = 1
'''
avail_thre__= np.zeros(10, dtype = float)
avail_thre__[0] = 0.85
...
for k in range(10):
	...
	m = range(0, num_vehicle + 1)
	plt.figure()
	plt.plot(m, min_avail)
	plt.xlabel('M : # of vehicles')
	plt.ylabel('min_avail')
	plt.show()
	'''

	argmax_m = np.argmax(profit)
	opti_m = -1
	for m in range(argmax_m, num_vehicle +1):
		if min_avail[m] >= avail_thre__[k] :
			opti_m = m 
			break
	...
	opti_m__[k] = opti_m
	if k <= 8:
		avail_thre__[k+1] = avail_thre__[k] + 0.01



plt.figure()
plt.plot(avail_thre__, opti_m__,'-b',linewidth=5.0)
plt.xticks(fontsize=30)
plt.yticks(fontsize=30)
plt.xlabel('Minimal Availability',fontsize = 40)
plt.ylabel('Optimal Number of Vehicles',fontsize = 40)
plt.show()
